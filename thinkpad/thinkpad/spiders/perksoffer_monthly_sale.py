...
from thinkpad.items import PerksOfferMonthlySaleItem
...
class PerksOfferMonthlySaleSpider(Spider):
    name = 'perksoffer_monthly_sale'
    allowed_domains = [tp.domain]

    def close(self, reason):
        tp.notify_user('Perks Offer Monthly Sale', self.name, self)
    ...
    def ok_perksoffer_login(self, response):
        yield Request(
            ...
            callback = self.ok_perksoffer_monthly_sale)

    def ok_perksoffer_monthly_sale(self, response):
        for product in self.get_perksoffer_monthly_sale_in_detail(response):
            yield product

    def get_perksoffer_monthly_sale_in_detail(self, response):
        ...
        for non_featured_product in non_featured_products:
            ...
            product = PerksOfferMonthlySaleItem()
            product['model'] = name
